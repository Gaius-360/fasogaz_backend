...
module.exports = (sequelize, DataTypes) => {
  const User = sequelize.define('User', {
    ...
    phone: {
      ...
      allowNull: false,
      // ✅ FIX : unique retiré ici, géré dans indexes (même raison qu'agentCode)
      validate: {
        is: /^\+226[0-9]{8}$/
      }
    },
    ...
    role: {
      type:         DataTypes.ENUM('client', 'revendeur', 'admin', 'agent'),
      defaultValue: 'client',
      allowNull:    false
    },
    profilePicture: {
    ...
    // ==========================================
    // CHAMPS AGENTS TERRAIN
    // ==========================================
    agentCode: {
      type:      DataTypes.STRING(20),
      allowNull: true,
      // ✅ FIX CRITIQUE : unique retiré ici
      // Avec alter:true, Sequelize génère "TYPE VARCHAR(20) UNIQUE" → syntaxe invalide en PostgreSQL
      // L'unicité est gérée proprement dans indexes ci-dessous avec une clause WHERE
      validate: {
        is: {
          args: /^AG-[A-Z0-9]{8}$/i,
    ...
    agentStats: {
      ...
      allowNull:    true,
      defaultValue: null,
      comment:      'Statistiques agent: { totalInvitationsSent, totalSellersRecruited, lastInvitationDate }'
    }
    ...
    indexes: [
      ...
      {
        // ✅ FIX : unique géré ici avec WHERE pour exclure les NULL
        // Sans WHERE, PostgreSQL traite tous les NULL comme égaux → erreur unique sur les non-agents
        unique: true,
        fields: ['agentCode'],
        name:   'idx_users_agentCode',
        where:  { agentCode: { [Op.ne]: null } }
      },
      {
  ...
  User.prototype.hasValidAccess = function() {
    ...
    if (!this.hasActiveAccess || !this.accessExpiryDate) return false;
  ...
  User.prototype.activateAccess = async function(durationHours = 24) {
    ...
    await this.save();
    return { activatedAt: now, expiresAt: expiryDate, durationHours };
  };
  ...
  User.prototype.updateAgentStats = async function(updates) {
    ...
    this.agentStats = { ...currentStats, ...updates };
    await this.save();
  ...
  User.associate = (models) => {
    ...
    User.hasMany(models.Order,       { foreignKey: 'customerId',  as: 'ordersAsCustomer' });
    User.hasMany(models.Order,       { foreignKey: 'sellerId',    as: 'ordersAsSeller' });
    User.hasOne( models.Subscription,{ foreignKey: 'userId',      as: 'subscription' });
    User.hasMany(models.Review,      { foreignKey: 'sellerId',    as: 'reviewsReceived' });
    User.hasMany(models.Review,      { foreignKey: 'customerId',  as: 'reviewsGiven' });
    User.hasMany(models.Transaction, { foreignKey: 'userId',      as: 'transactions' });
    User.hasMany(models.AccessPurchase, { foreignKey: 'userId',   as: 'accessPurchases' });
